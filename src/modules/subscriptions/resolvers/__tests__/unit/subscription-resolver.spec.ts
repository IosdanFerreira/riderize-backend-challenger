...
describe("SubscriptionResolver", () => {
  ...
  beforeEach(() => {
    ...
    subscriptionResolver = new SubscriptionResolver(subscriptionServiceMock);
    jest.clearAllMocks();
  });
  ...
  describe("subscribeToRide", () => {
    it("deve inscrever usuário no pedal com sucesso", async () => {
      ...
      const fakeSubscription: SubscriptionModel = {
        id: "sub-1",
        ride_id,
        user_id: fakeUserId,
        created_at: new Date(),
      ...
      subscriptionServiceMock.subscribeToRide.mockResolvedValue(
        fakeSubscription
      );

      const context = { user: { id: fakeUserId } };
      const result = await subscriptionResolver.subscribeToRide(
        ride_id,
        context as any
      );

      expect(subscriptionServiceMock.subscribeToRide).toHaveBeenCalledWith(
        ride_id,
        fakeUserId
      );
    ...
    it("deve propagar erro do serviço caso falhe a inscrição", async () => {
      ...
      await expect(
        subscriptionResolver.subscribeToRide(ride_id, context as any)
      ).rejects.toThrow("Erro ao inscrever");
  ...
  describe("mySubscribedRides", () => {
    it("deve retornar lista de pedais inscritos do usuário", async () => {
      ...
      subscriptionServiceMock.getAllRidesSubscribedByUser.mockResolvedValue(
        rides
      );

      const context = { user: { id: fakeUserId } };
      const result = await subscriptionResolver.mySubscribedRides(
        context as any
      );

      expect(
        subscriptionServiceMock.getAllRidesSubscribedByUser
    ...
    it("deve propagar erro do serviço ao buscar pedais inscritos", async () => {
      ...
      subscriptionServiceMock.getAllRidesSubscribedByUser.mockRejectedValue(
        error
      );
      ...
      await expect(
        subscriptionResolver.mySubscribedRides(context as any)
      ).rejects.toThrow("Erro ao buscar inscritos");
