...
describe("SubscriptionService Unit Tests", () => {
  ...
  describe("subscribeToRide", () => {
    it("deve inscrever usuário com sucesso", async () => {
      ...
      subscriptionRepository.create.mockResolvedValue({ id: "sub-1" } as any);

      const result = await subscriptionService.subscribeToRide(
        "ride-1",
        "user-1"
      );

      expect(rideRepository.findById).toHaveBeenCalledWith("ride-1");
      expect(subscriptionRepository.findFirst).toHaveBeenCalledWith(
        "ride-1",
        "user-1"
      );
    ...
    it("deve lançar NotFoundError se ride não existir", async () => {
      ...
      await expect(
        subscriptionService.subscribeToRide("ride-1", "user-1")
    ...
    it("deve lançar erro se período de inscrição ainda não começou", async () => {
      ...
      await expect(
        subscriptionService.subscribeToRide("ride-1", "user-1")
      ).rejects.toThrow("Período de inscrição ainda não começou");
    });
    ...
    it("deve lançar erro se período de inscrição já acabou", async () => {
      ...
      await expect(
        subscriptionService.subscribeToRide("ride-1", "user-1")
      ).rejects.toThrow("Período de inscrição encerrado");
    });
    ...
    it("deve lançar erro se usuário já estiver inscrito", async () => {
      ...
      await expect(
        subscriptionService.subscribeToRide("ride-1", "user-1")
      ).rejects.toThrow("Usuário já está inscrito nesse pedal");
    });
    ...
    it("deve lançar erro se limite de participantes for atingido", async () => {
      ...
      await expect(
        subscriptionService.subscribeToRide("ride-1", "user-1")
      ).rejects.toThrow("Limite de participantes atingido");
    });
  });
  ...
  describe("getAllRidesSubscribedByUser", () => {
    it("deve retornar lista de rides a partir das inscrições do usuário", async () => {
      ...
      const result = await subscriptionService.getAllRidesSubscribedByUser(
        "user-1"
      );

      expect(subscriptionRepository.findManyByUserId).toHaveBeenCalledWith(
        "user-1"
      );
