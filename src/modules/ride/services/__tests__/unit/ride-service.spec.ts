...
describe("RideService unit tests", () => {
  ...
  describe("createRide", () => {
    ...
    it("deve lançar ValidationError se a data final de inscrição for no passado", async () => {
      ...
      await expect(rideService.createRide(input, "user-1")).rejects.toThrow(
        ValidationError
      );
    });
  ...
  describe("updateRide", () => {
    it("deve atualizar a ride quando usuário for criador", async () => {
      ...
      const input: UpdateRideInput = { name: "Novo nome" } as any;
    ...
    it("deve lançar NotFoundError se ride não existir", async () => {
      ...
      await expect(
        rideService.updateRide("ride-1", {} as any, "user-1")
    ...
    it("deve lançar ValidationError se novas datas forem inválidas", async () => {
      ...
      await expect(
        rideService.updateRide("ride-1", input, "user-1")
  ...
  describe("deleteRide", () => {
    ...
    it("deve lançar NotFoundError se ride não existir", async () => {
      ...
      await expect(rideService.deleteRide("ride-1", "user-1")).rejects.toThrow(
        NotFoundError
      );
    });
  ...
  describe("getRidesCreatedByUser", () => {
    ...
    it("deve lançar NotFoundError se não houver rides", async () => {
      rideRepository.findManyByCreatorId.mockResolvedValue(null as any);

      await expect(rideService.getRidesCreatedByUser("user-1")).rejects.toThrow(
        NotFoundError
      );
    });
  });
  ...
  describe("getAllRidesPaginated", () => {
    it("deve retornar paginação corretamente", async () => {
      ...
      (Paginator.buildPagination as jest.Mock).mockReturnValue({
        pagination: {
          total_items: 1,
          page: 1,
          per_page: 10,
      ...
      expect(Paginator.buildPagination).toHaveBeenCalledWith([fakeRide], 1, {
        page: 1,
        per_page: 10,
      });
  ...
  describe("getRideById", () => {
    ...
    it("deve lançar NotFoundError se ride não existir", async () => {
      ...
      await expect(rideService.getRideById("ride-1")).rejects.toThrow(
        NotFoundError
      );
    });
  });
