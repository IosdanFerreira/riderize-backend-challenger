...
describe("RideResolver", () => {
  let rideResolver: RideResolver;
  let rideServiceMock: jest.Mocked<RideService>;
  ...
  beforeEach(() => {
    ...
    rideResolver = new RideResolver(rideServiceMock);
    jest.clearAllMocks();
  });
  ...
  describe("createRide", () => {
    it("deve criar um novo pedal com sucesso", async () => {
      const input = { name: "Novo Pedal" } as any;
      ...
      expect(rideServiceMock.createRide).toHaveBeenCalledWith(
        input,
        fakeUserId
      );
  ...
  describe("deleteRide", () => {
    it("deve deletar um pedal com sucesso e retornar true", async () => {
      ...
      expect(rideServiceMock.deleteRide).toHaveBeenCalledWith(
        ride_id,
        fakeUserId
      );
  ...
  describe("myCreatedRides", () => {
    it("deve retornar lista de pedais criados pelo usuário", async () => {
      ...
      expect(rideServiceMock.getRidesCreatedByUser).toHaveBeenCalledWith(
        fakeUserId
      );
      expect(result).toEqual([fakeRide]);
    });
  ...
  describe("getAllRides", () => {
    it("deve retornar pedais paginados", async () => {
      const page = 1;
      const limit = 10;
      const paginatedResult: PaginatedRides = {
        items: [fakeRide],
        pagination: {
          total_items: 1,
          page,
      ...
      expect(rideServiceMock.getAllRidesPaginated).toHaveBeenCalledWith(
        page,
        limit
      );
